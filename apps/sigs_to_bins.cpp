...
int main(int argc, char** argv) {
    ...
    for (const auto& dirEntry : glob2(sigs_dir + "/*")) {
        ...
        std::string sig_basename = file_name.substr(file_name.find_last_of("/\\") + 1);
        string sig_prefix = file_name.substr(0, lastindex);
        std::string::size_type idx;
        idx = file_name.rfind('.');
        std::string extension = "";
        if (idx != std::string::npos) extension = file_name.substr(idx + 1);
        // TODO: Allow duplicate names with different gz_xx
        if (extension.find("sig") == std::string::npos && extension.find("gz") == std::string::npos) {
            cerr << "skipping ..." << endl;
            continue;
        }
